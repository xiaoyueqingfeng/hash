<template>
  <oa-modal v-model="show" title="查看账户" :footer="null">
    <div class="account">
      <ul>
        <li v-for="(item, key) in suppluerShow" :key="key">
...
<script>
import { getBank } from '@/api/suppliers'
  ...
  computed: {
    suppluerShow() {
      return [
        {
          label: '供应商名称',
          value: get(this.account, 'supplier_name', '')
        },
        {
          label: '帐户名',
