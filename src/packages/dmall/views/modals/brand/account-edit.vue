<template>
  <oa-modal title="编辑品牌商账号" v-model="show" @ok="onSubmit">
    <a-row>
      <div class="form">
        <a-form :form="form">
          <a-form-item label="品牌商名称" v-bind="formCol">
            {{ account.brand_dealer_name }}
          </a-form-item>
...
<script>
import { updateBank } from '@/api/brand'
  ...
  methods: {
    onSubmit() {
      this.form.validate().then(values => {
        const form = {
          brand_dealer_id: this.account.id,
          ...values
        }
        this.show = false
