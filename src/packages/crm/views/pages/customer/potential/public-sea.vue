<template>
  <div class="potential-public-sea">
    ...
    <oa-table
      ...
      rowKey="id"
      :loading="getPotentialPublicList$loading"
...
<script>
import { getPotentialPublicList } from '@/api/potential'
...
export default {
  ...
  async: {
    initData() {
      return Promise.all([this.getPotentialPublicList()])
    },
    getPotentialPublicList() {
      return getPotentialPublicList(this.$searchQuery).then(res => {
  ...
  methods: {
    onModalDone() {
      this.getPotentialPublicList().then(res => {
