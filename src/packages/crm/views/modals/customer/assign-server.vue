          ...
          >
            <oa-select
              ...
              v-decorator="[
                'server_id',
                {
                  rules: [{ required: true, message: '客运必选' }]
                }
              ]"
            ></oa-select>
          </a-form-item>
...
<script>
import { getOrganList, getOperatorList, assignCustomServer } from '@/api/pay'
...
export default {
  ...
  async: {
    getServerList(query) {
      switch (this.from) {
        case 'pay': {
          return getOperatorList(query).then(res => {
    ...
    assignServer(data) {
      switch (this.from) {
        case 'pay':
          return assignCustomServer(data)
      }
    }
  },
  methods: {
    onSubmit() {
      this.form.validateFields((err, fieldsValue) => {
        if (!err) {
          const sendData = {
            custom_operation_server_id: fieldsValue.server_id,
            leads_id: this.customerIds
          }
          this.assignServer(sendData).then(() => {
            this.$message.success('分配客运成功！')
            this.$emit('done')
            this.show = false
          })
        }
